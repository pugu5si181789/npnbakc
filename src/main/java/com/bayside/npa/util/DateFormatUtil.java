...
public class DateFormatUtil {
    ...
    */
    ...
    public static String getPubdate(String pubdate){
		if(pubdate==null){
			return null;
		}
		String resultdate = null;
		pubdate = pubdate.replace("/", "-").replace("年", "-").replace("月", "-").replace("日", " ");
		pubdate=pubdate.replaceAll(" +", " ");
		Date date = stringFormatDateType(pubdate, "yyyy-MM-dd HH:mm:ss");
		if(date==null){
			date = stringFormatDateType(pubdate, "yyyy-MM-dd HH:mm");
		}
		if(date == null){
			date = stringFormatDateType(pubdate, "yyyy-MM-dd");
		}
		if(date == null){
			date = stringFormatDateType(((new Date()).getYear()+1900)+"-"+pubdate, "yyyy-MM-dd HH:mm");
		}
		if(date == null){
			date = stringFormatDateType(pubdate, "yyyyMMdd");
		}
		if(date!=null){
			resultdate = dateFormatStringType(date, "yyyy-MM-dd HH:mm:ss");
		}
		return resultdate;
	}
   	/**
    ...
    public static void main(String[] args) throws ParseException {
    	List<String> list = getDateList("2016-11-25","2016-12-01",15);
